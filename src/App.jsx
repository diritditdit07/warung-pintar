...
function getInitialPreLoginScreen() {
  const path = window.location.pathname.toLowerCase();
  if (path === '/add_user' || path === '/add_warung' || path === '/add-user' || path === '/add-warung') {
    return 'add-store';
  }
  return 'login';
}
...
export default function App() {
  ...
  const [preLoginScreen, setPreLoginScreen] = useState(getInitialPreLoginScreen);
  ...
  async function handleDeleteSale(saleId) {
    const confirmed = window.confirm('Yakin hapus transaksi penjualan ini?');
    if (!confirmed) {
      return;
    }

    try {
      await invokeWarungAction(authSession, 'delete-sale', { id: saleId });
      setSales((currentSales) => currentSales.filter((sale) => sale.id !== saleId));
      setToastMessage('Transaksi penjualan berhasil dihapus');
    } catch (error) {
      setToastMessage(error instanceof Error ? error.message : 'Gagal hapus transaksi penjualan');
    }
  }

  async function handleDeleteExpense(expenseId) {
    const confirmed = window.confirm('Yakin hapus pengeluaran ini?');
    if (!confirmed) {
      return;
    }

    try {
      await invokeWarungAction(authSession, 'delete-expense', { id: expenseId });
      setExpenses((currentExpenses) => currentExpenses.filter((expense) => expense.id !== expenseId));
      setToastMessage('Pengeluaran berhasil dihapus');
    } catch (error) {
      setToastMessage(error instanceof Error ? error.message : 'Gagal hapus pengeluaran');
    }
  }
  ...
  async function handleDeleteProduct(productId) {
    const confirmed = window.confirm('Yakin hapus produk ini? Data penjualan lama tetap akan disimpan.');
    if (!confirmed) {
      return;
    }

    try {
      await invokeWarungAction(authSession, 'delete-product', { id: productId });
      ...
      setToastMessage(error instanceof Error ? error.message : 'Gagal hapus produk');
    }
  }
  ...
  if (!authSession) {
    ...
    if (preLoginScreen === 'add-store') {
      return (
        <main className="app-shell">
          <div className="app-frame">
            <AddStorePage onBack={() => {
              window.history.replaceState({}, '', '/');
              setPreLoginScreen('login');
            }} />
          </div>
        </main>
      );
