import { getWarungLoginUrl, getWarungCreateStoreUrl, hasSupabaseEnv, supabaseAnonKey } from './supabase';
...
export async function createStore(storeCode, storeName, pin, fullName) {
  if (!hasSupabaseEnv()) {
    throw new Error('Supabase env belum diisi. Tambahkan VITE_SUPABASE_URL dan VITE_SUPABASE_ANON_KEY.');
  }

  const response = await fetch(getWarungCreateStoreUrl(), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      apikey: supabaseAnonKey,
    ...
    body: JSON.stringify({ storeCode, storeName, pin, fullName })
  });

  const payload = await response.json().catch(() => ({}));

  if (!response.ok) {
    throw new Error(payload.error || 'Gagal membuat warung');
  }
